...
/// Log levels for the EMWDAT module
public enum EMWDATLogLevel: Int, Comparable {
    case debug = 0
    case info = 1
    case warn = 2
    case error = 3
    ...
    public static func < (lhs: EMWDATLogLevel, rhs: EMWDATLogLevel) -> Bool {
...
/// Logger for the EMWDAT module
/// Format: [EMWDAT] [LEVEL] [Component] Message - {context}
public final class EMWDATLogger {
    public static let shared = EMWDATLogger()

    private let osLog = OSLog(subsystem: "expo.modules.emwdat", category: "EMWDAT")
    private var currentLevel: EMWDATLogLevel = .info
    private let queue = DispatchQueue(label: "expo.modules.emwdat.logger")

    private init() {}

    /// Set the minimum log level
    public func setLogLevel(_ level: EMWDATLogLevel) {
        queue.sync {
            currentLevel = level
        }
    }
    ...
    public func getLogLevel() -> EMWDATLogLevel {
    ...
    private func log(_ level: EMWDATLogLevel, component: String, message: String, context: [String: Any]?) {
        guard level >= getLogLevel() else { return }

        var logMessage = "[EMWDAT] [\(level.prefix)] [\(component)] \(message)"
